...
if __name__ == '__main__':
    ...
    file = open(path + 'trained_data', 'wb')
    pickle.dump(train_data, file)

    file = open(path + 'test_data', 'wb')
    pickle.dump(test_data, file)

    file = open(path + 'trained_label', 'wb')
    pickle.dump(train_label, file)

    file = open(path + 'test_label', 'wb')
    ...
    weight = {}
    # weight['con1']= np.full((hidden_neurons,num_terminals *input_neurons), 1.)
    # weight['con2']= np.full((output_neurons,num_terminals*hidden_neurons), 1.)

    weight['con1'] = np.random.uniform(0.9, 1.1, (hidden_neurons, num_terminals * input_neurons))
    weight['con2'] = np.random.uniform(0.9, 1.1, (output_neurons, num_terminals * hidden_neurons))
